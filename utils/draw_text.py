...
def draw_title_text(overlay, TITLE_TEXT, FONT_OPTION, MAX_TITLE_WIDTH, min_lines, alignment, text_outline_width, text_outline_color):
    ...
    font = REGULAR_FONT if FONT_OPTION == 'regular' else BOLD_FONT

    title_lines = wrap_title_text(overlay_draw, TITLE_TEXT, MAX_TITLE_WIDTH, min_lines, font)
    title_y = calculate_title_position(overlay_draw, title_lines, FONT_OPTION)

    for line in title_lines:
        draw_outlined_text(overlay_draw, line, title_y, FONT_OPTION, alignment, text_outline_width, text_outline_color)
        title_y += get_line_height(overlay_draw, line, FONT_OPTION) + 10  # Adjust the spacing between lines
...
def wrap_title_text(draw, text, max_width, min_lines=1, font=REGULAR_FONT):
    lines = []
    current_line = ""
    words = text.split()
...
def calculate_title_position(draw, title_lines, FONT_OPTION):
    title_height = sum(get_line_height(draw, line, FONT_OPTION) for line in title_lines)
...
def calculate_subtitle_position(draw, subtitle_lines):
    title_height = sum(get_line_height(draw, line, FONT_OPTION) for line in TITLE_TEXT.split())
    title_spacing = 10  # Spacing between title lines
    ...
    subtitle_y = (HEIGHT - total_text_height) // 2 + total_title_height + 30
    return subtitle_y

def draw_outlined_text(draw, text, y, FONT_OPTION, alignment, outline_width, outline_color):
    font = REGULAR_FONT if FONT_OPTION == 'regular' else BOLD_FONT
    ...
    if alignment == 'left':
        ...
        x = WIDTH - MAX_TITLE_WIDTH - 20  # Adjust the value 20 to change the right margin
...
def draw_text(draw, text, y, font, alignment):
    ...
    if alignment == 'left':
        ...
        x = WIDTH - line_width - 20  # Adjust the value 20 to change the right margin
    elif alignment == 'center':
        x = (WIDTH - line_width) // 2
    else:
        raise ValueError(f"Invalid alignment option: {alignment}")
